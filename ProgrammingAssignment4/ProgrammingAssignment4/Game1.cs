...
namespace ProgrammingAssignment4
{
	...
	public class Game1 : Game
	{
		...
		protected override void LoadContent ()
		{
			...
			teddy = new TeddyBear(teddySprite, new Vector2((WindowWidth) / 2,
			                                               (WindowHeight) / 2));
		}
		...
		protected override void Update (GameTime gameTime)
		{
			...
			else if (mouse.RightButton == ButtonState.Released)
			{
				...
				if (rightClickStarted)
				{
					...
					var pickupLocation = new Vector2(mouse.X, mouse.Y);
				...
				teddy.CollisionRectangle.Intersects(pickups[0].CollisionRectangle))
			{
				...
				if (pickups.Count > 0)
				{
					// reconstruct the pickup location from pickup's draw rectangle
					// we need to add the half of pickup's width and height to get location of center
					Rectangle pickupRectangle = pickups[0].CollisionRectangle;
					int pickupLocationX = pickupRectangle.X + pickupSprite.Width / 2;
					int pickupLocationY = pickupRectangle.Y + pickupSprite.Height / 2;
					teddy.SetTarget(new Vector2(pickupLocationX, pickupLocationY));
				}
				else 
				{
					// stop teddy from collecting
					teddy.ClearTarget();
